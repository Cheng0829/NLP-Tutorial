...
class MyDataSet(Data.Dataset):
    ...
    def __getitem__(self, idx):
        # print(self.enc_inputs[idx], self.dec_inputs[idx], self.dec_outputs[idx])
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        pe[:, 1::2] = torch.cos(position * div_term) # 按照公式计算
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self):
        ...
        self.W_Q = nn.Linear(d_model, d_k * n_heads, bias=False)  # Q和K维度必须相同，不然无法做点积
        self.W_K = nn.Linear(d_model, d_k * n_heads, bias=False)
        self.W_V = nn.Linear(d_model, d_v * n_heads, bias=False)
        # 这个全连接层可以保证多头attention的输出仍然是seq_len * d_model
...
class PoswiseFeedForwardNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        """
        residual = inputs
        output = self.fc(inputs) # ReLU
        return nn.LayerNorm(d_model).to(device)(output + residual)  # [batch_size, seq_len, d_model]

class EncoderLayer(nn.Module):
    def __init__(self):
        super(EncoderLayer, self).__init__()
        self.enc_self_attn = MultiHeadAttention() # 包含Norm
        self.pos_ffn = PoswiseFeedForwardNet() # MLP
    ...
    def forward(self, enc_inputs, enc_self_attn_mask):
        ...
        enc_outputs = self.pos_ffn(enc_outputs) # MLP
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.ModuleList([EncoderLayer() for _ in range(n_layers)]) # 把结果编码器块串在一起
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        # torch.gt(a,b)函数比较a中和b中对应元素，大于则为1，不大于则为0，这里a为Tensor，b可以为与a的size相同的Tensor或常数。
...
class Transformer(nn.Module):
    ...
    def forward(self, enc_inputs, dec_inputs):
        ...
        enc_outputs, enc_self_attns = self.encoder(enc_inputs) # XXX_self_attns用于打印参数,和实际训练没关系
...
# 预测阶段
# 测试集
sentences = [['我 有 零 个 女 朋 友 P', '', '']] # enc_input dec_input   ec_output
enc_inputs, dec_inputs, dec_outputs = make_data(sentences)
...
for i in range(len(enc_inputs)): # 遍历测试文本集,一个enc_inputs就是一个句子
    greedy_dec_predict = greedy_decoder(model, 
        enc_inputs[i].view(1, -1).to(device), start_symbol = tgt_vocab["S"])
