...
残差块⾥首先有2个有相同输出通道数的3x3卷积层。每个卷积层后接⼀个批量归⼀化层和ReLU激活函数。然后我们将输⼊跳过这两个卷积运算后直接加在最后的ReLU激活函数前。这样的设计要求**两个卷积层的输出与输⼊形状⼀样**，从⽽可以相加。如果想改变通道数，就需要引⼊⼀个额外的1x1卷积层来将输⼊变换成需要的形状后再做相加运算
![ ](img/ResNet.jpg)
